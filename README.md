...
| Temp-Namespace-Operator | [CognotektGmbH/tmp-namespace-operator](https://github.com/CognotektGmbH/tmp-namespace-operator) | The Temp-Namespace-Operator creates namespaces on-the-fly with proper security resources (networkpolicies, rolebindings, etc.) in it and a TTL until they get deleted. This is useful for running integration tests automatically in insolated production-like environment. It allows developers to set up compliant namespaces temporarily for their works without giving them the permission to modify actual Kubernetes namespace resources.|
