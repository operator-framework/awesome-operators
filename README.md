...
| VaultSync Operator | [thatInfrastructureGuy/vaultsync-operator](https://github.com/thatInfrastructureGuy/vaultsync-operator) | Periodically syncs secrets from industry standard Vaults such as Azure KeyVault / AWS Secrets Manager to Kubernetes Secrets. This operator can redeploy your deployments/statefulsets to help end-user application retrieve the newly updated secrets when kubernetes secrets are mounted as volume or environment variables).  |
